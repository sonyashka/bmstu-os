...
struct sembuf p_consumer[2] = {{BUFER_FULL, -1, 0}, {BIN_SEM, -1, 0}};
struct sembuf v_consumer[2] = {{BIN_SEM, 1, 0}, {BUFER_EMPTY,  1, 0}};

struct sembuf p_producer[2] = {{BUFER_EMPTY, -1, 0}, {BIN_SEM, -1, 0}};
struct sembuf v_producer[2] ={{BIN_SEM, 1, 0}, {BUFER_FULL, 1, 0}};
...
if ((shm_id = shmget(IPC_PRIVATE, N * sizeof(char) + 3 * sizeof(int), IPC_CREAT | flags)) == -1)
    ...
    semctl(sem_id, BUFER_EMPTY, SETVAL, N);
    semctl(sem_id, BUFER_FULL, SETVAL, 0);
